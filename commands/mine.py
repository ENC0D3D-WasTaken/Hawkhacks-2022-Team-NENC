...
async def mine(ctx):
    ...
    if msg.content == str(answer):
        ...
        winmsg = await ctx.send(embed=winmsg)
        await deleteMessage(winmsg)
        correct = True
        db.execute(f'UPDATE users SET balance = balance + {payout} WHERE memberId = {ctx.message.author.id}')
    else:
        ...
        losemsg = await ctx.send(embed=losemsg)
        db.execute(f'UPDATE users SET balance = balance - {payout} WHERE memberId = {ctx.message.author.id}')
        await deleteMessage(losemsg)
    try:
        db.commit()
